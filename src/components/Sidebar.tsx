...
const Sidebar = (props: ISidebarProps) => {
  ...
  return (
    <VerticalSidebar>
			<IconButtonBavard onClick={() => console.log('bavard_icon')} tooltip="tooltip"/>
			{
				items.map(item => <NavItem active={selected === item.key} keyVal={item.key} path={item.path} css={item.css} onClick={item.handler}/>)
			}      
    </VerticalSidebar>
  );
};
