...
const CreateTrainingConversations: React.FC<IConversationProps> = ({conversationLastindex, onSaveCallback, isUpdate,  conversation, onCancel }) => {
  ...
  if (isUpdate && conversation && conversation.actions) {
    ...
    conversation.actions.map((i: any) => userTurns[i.turn] = i.isUser ? 'user' : 'agent');
  }
  ...
  let actionId = actions !== undefined ?  actions?.map(item => ({id: item.id, value: item.name, text: item.text})) : [];
  ...
  const handleAddFields = (turnValue: string) => {
    ...
    if (turnValue === 'user') {
      ...
      values.push({
        agentActions: [{ turn:  0, actionId: '', actionType: '', utterance: '' }] ,
      });
    }
  ...
  return (
    <Paper className={classes.paper}>
      {errStatus && <Alert severity="error" className={classes.progressIndicator}>{errStatus}</Alert>}
