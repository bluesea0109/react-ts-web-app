...
const CreateTrainingConversations: React.FC<IConversationProps> =
  ({ conversationLastindex, onSaveCallback, isUpdate, conversation, onCancel }) => {
    ...
    const actionId = actions !== undefined ? actions?.map(item => ({ id: item.id, value: item.name, text: item.text })) : [];
